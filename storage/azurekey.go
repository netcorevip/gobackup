...
func (s *AzureKey) open() error {
	...
	accountkey := s.viper.GetString("accountkey")

	credential, err := azblob.NewSharedKeyCredential(s.account, accountkey)
	//credential, err := azidentity.NewClientSecretCredential(tenantId, clientId, clientSecret, nil)
	if err != nil {
		return fmt.Errorf("Invalid credentials with error: %w", err)
	}
	s.client, err = azblob.NewClientWithSharedKeyCredential(s.getBucketURL(), credential, nil)
...
func (s *AzureKey) close() {
}

func (s AzureKey) getBucketURL() string {
	return fmt.Sprintf("https://%s.blob.core.windows.net", s.account)
}

func (s *AzureKey) upload(fileKey string) (err error) {
	logger := logger.Tag("AzureKey")
...
func (s *AzureKey) delete(fileKey string) (err error) {
...
func (s *AzureKey) list(parent string) ([]FileItem, error) {
...
func (s *AzureKey) download(fileKey string) (string, error) {
